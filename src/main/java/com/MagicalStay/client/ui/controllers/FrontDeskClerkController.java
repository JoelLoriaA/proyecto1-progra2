...
public class FrontDeskClerkController {
    ...
    public FrontDeskClerkController() {
        socketCliente = new SocketCliente(new SocketCliente.ClienteCallback() {
            @Override
            public void onMensajeRecibido(String mensaje) {
                if (mensaje.startsWith("NOTIFY|frontdesk_update")) {
                    // Recargar la lista de recepcionistas desde el servidor
                    javafx.application.Platform.runLater(() -> loadClerksFromFile());
                }
            }
            @Override
            public void onError(String error) {
                javafx.application.Platform.runLater(() -> showError("Error de comunicación: " + error));
            }
            @Override
            ...
            public void onDesconexion() {
                javafx.application.Platform.runLater(() -> showError("Desconectado del servidor"));
            }
        });
    }
